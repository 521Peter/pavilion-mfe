...
export function Sidebar({ openSettingDrawer }: { openSettingDrawer: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  const removeCustomerGroup = (target: string) => {
    setCustomerGroups(currentGroups => currentGroups.filter(item => item !== target));
  };
                      ...
                      customerGroups.map((item, index) => (
                        ...
                        >
                          ...
                          <span className="flex items-center gap-1 text-xs text-slate-400">
                            ...
                            <Button
                              isIconOnly
                              aria-label={`删除客户群体：${item}`}
                              onPress={() => removeCustomerGroup(item)}
                              size="sm"
                              variant="secondary"
                            >
                              <Icon name="Trash2" />
