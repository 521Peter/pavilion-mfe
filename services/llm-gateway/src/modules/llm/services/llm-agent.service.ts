...
export class LlmAgentService {
  ...
  async *stream(model: BaseChatModel, params: ChatParams, execution?: AgentExecutionConfig): AsyncGenerator<string> {
    ...
    // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- LangGraph streamMode=messages yields documented [BaseMessage, metadata] tuples but its generic return type does not preserve the selected mode
