...
describe("gateway security boundaries", () => {
  ...
  it("rejects persisted credential payloads with an invalid shape", () => {
    const config = new ConfigService({ app: { credentialEncryptionKey: Buffer.alloc(32, 7).toString("base64") } });
    const service = new SecretEncryptionService(config);
    const invalidPayload = Buffer.from(JSON.stringify({ version: 1, iv: "missing-fields" }), "utf8").toString("base64");
    expect(() => service.decrypt(invalidPayload)).toThrow("Invalid credential payload");
  });
