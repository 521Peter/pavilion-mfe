...
export default function Env() {
  ...
  const envRows: { key: string; label: string; value: React.ReactNode; label2?: string; value2?: React.ReactNode }[] = [
    {
      key: 'env',
      label: '当前环境',
      value: <Chip color={envTagColor} variant="soft">{pavilionMfeEnv}</Chip>,
      label2: 'API Base',
      value2: apiBase || '-',
    },
    {
  ...
  const runtimeRows: { key: string; label: string; value: React.ReactNode; label2?: string; value2?: React.ReactNode }[] = [
    {
      key: 'path',
      label: '当前路径',
      value: currentPath,
  ...
  return (
    <div>
      <h1 className="text-[22px] font-extrabold text-text-primary m-0 mb-6 tracking-[-0.3px]">
        环境信息
      </h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4 max-[900px]:grid-cols-1">
        {infoCards.map((item) => (
          <Card key={item.label} variant="default" className="flex flex-row items-center justify-between p-5">
            ...
            <Chip color={item.color} variant="soft">{item.value}</Chip>
          </Card>
        ))}
      </div>
      ...
      <Card variant="default" className="p-5 mb-4">
        ...
        <Table>
          <Table.ScrollContainer className="overflow-x-auto">
            <Table.Content>
              <Table.Header>
                <Table.Column id="code" isRowHeader>菜单编码</Table.Column>
      ...
      <Card variant="default" className="p-5 mb-4">
        ...
        <Table>
          <Table.Content>
            <Table.Header>
              <Table.Column id="k1" isRowHeader>键</Table.Column>
