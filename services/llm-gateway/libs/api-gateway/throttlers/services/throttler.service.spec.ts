...
describe("ThrottlerService keyResolver", () => {
  describe("custom keyResolver — non-empty return", () => {
    it("buckets by the resolved identity (same identity shares a bucket; different identities are independent)", async () => {
      ...
      await expect(service.checkLimitOfRequest(routerDetail, userA)).rejects.toBeInstanceOf(TooManyRequestException);
      ...
      const service = await bootService(
        {
          ...
          keyResolver: async ({ request }) => {
            ...
            return `tenant:${String(request.headers["x-tenant"])}`;
          }
        },
        redis
      );
  ...
  describe("skip-on-empty", () => {
    ...
    it("exempt requests do not consume the custom bucket used by non-exempt requests", async () => {
      ...
      await expect(service.checkLimitOfRequest(routerDetail, makeRequest({ url: "/things" }))).rejects.toBeInstanceOf(
  ...
  describe("global IP rate limit", () => {
    it("always applies based on request.ip, even when keyResolver returns null (skip)", async () => {
      ...
      await service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "5.5.5.5" }));
      await expect(service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "5.5.5.5" }))).rejects.toBeInstanceOf(
        TooManyRequestException
      );
      ...
      const service = await bootService(
        {
          ...
          globalIpRateLimitTTL: 60,
          ...HIGH_CUSTOM_LIMIT,
          keyResolver: ({ request }) => `user:${String(request.headers["auth-user-id"])}`
        },
        redis
      );

      await service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "7.7.7.7", headers: { "auth-user-id": "a" } }));
      await service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "7.7.7.7", headers: { "auth-user-id": "b" } }));
  ...
  describe("no keyResolver configured — backwards compatibility", () => {
    it("falls back to request.ip for the custom bucket (in addition to the IP bucket)", async () => {
      ...
      await service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "1.2.3.4" }));
      await expect(service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "1.2.3.4" }))).rejects.toBeInstanceOf(
        TooManyRequestException
      );

      await expect(service.checkLimitOfRequest(routerDetail, makeRequest({ ip: "9.9.9.9" }))).resolves.toBeUndefined();
