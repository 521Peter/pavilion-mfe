...
import { jsonObjectOrEmpty, toPrismaJson } from "@/database/prisma-json";
...
export class LlmProviderService {
  ...
  async getModel(providerId: string, modelId: string): Promise<BaseChatModel> {
    ...
    const providerConfig: ProviderConfig = {
      ...
      apiKey: provider.credentials[0] ? this.secrets.decrypt(provider.credentials[0].encryptedPayload) : undefined,
      extra: jsonObjectOrEmpty(provider.config)
    };

    const modelConfig: ModelConfig = {
      ...jsonObjectOrEmpty(model.config),
    ...
    const instance = ProviderFactory.create(
      {
        ...
        extra: jsonObjectOrEmpty(deployment.provider.config)
      },
      { ...jsonObjectOrEmpty(deployment.config), modelName: deployment.upstreamModel }
    );
    this.modelCache.set(cacheKey, instance);
    return instance;
  }
