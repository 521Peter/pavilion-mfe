...
import { Drawer, Button, Label, Input, Separator as Divider, type DrawerProps } from "@heroui/react";
import { useState } from "react";

function SettingDrawer({ isOpen, onOpenChange }: Partial<DrawerProps>) {
  return (
...
function App() {
  ...
  return (
    <>
        ...
        <Panel className="h-full" defaultSize="20%" minSize="20%">
          <section className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white text-sm font-medium text-slate-500 shadow-sm">
            <div className="flex items-center justify-between p-3">
              ...
              <Button isIconOnly aria-label="设置" variant="secondary" onPress={() => setShowSettingDrawer(true)}>
                <Icon name="Bolt" />
              </Button>
            </div>
      ...
      <SettingDrawer isOpen={showSettingDrawer} onOpenChange={setShowSettingDrawer} />
    </>
  );
}
