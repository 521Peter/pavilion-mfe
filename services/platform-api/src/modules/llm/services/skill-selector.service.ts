...
export function parseSelectedSkillNames(raw: string, availableNames: ReadonlySet<string>): string[] {
...
export class SkillSelectorService {
  ...
  async select(model: BaseChatModel, messages: ChatMessage[]): Promise<SelectedSkill[]> {
    try {
      const skills = (await this.skillService.list()).filter(skill => skill.isActive && skill.description.trim());
      ...
      this.logger.warn(`Skill 自动选择失败，本轮回退为普通聊天: ${message}`);
      return [];
    }
  }
}
