...
export class SkillService {
  ...
  private async fetchGithubTree(
    ...
    if (!Array.isArray(entries)) throw new Error("GitHub API 目录响应必须是数组");
    const files: { path: string; downloadUrl: string }[] = [];

    for (const entry of entries) {
      if (typeof entry !== "object" || entry === null || !("type" in entry) || !("path" in entry)) continue;
      if (typeof entry.path !== "string") continue;
      if (entry.type === "file" && "download_url" in entry && typeof entry.download_url === "string") {
