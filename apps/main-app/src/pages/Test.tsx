...
export default function Test() {
  return (
    <div>
      <h1 className="text-[22px] font-extrabold text-text-primary m-0 mb-1.5 tracking-[-0.3px]">
        测试页
      </h1>
      <p className="text-sm text-text-regular m-0 mb-6">
        这是主应用自带的页面，不经过子应用加载。
      </p>
      ...
      <Card variant="default" className="p-5 mb-4">
        ...
        <Table>
          <Table.Content>
            <Table.Header>
              <Table.Column id="label" isRowHeader>项目</Table.Column>
      ...
      <Card variant="default" className="p-5">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="primary" onPress={() => navigateTo('/demo/list')}>Vue 列表页</Button>
          <Button variant="outline" onPress={() => navigateTo('/demo/form')}>Vue 表单页</Button>
          <Button variant="primary" onPress={() => navigateTo('/react/list')}>React 列表页</Button>
          <Button variant="outline" onPress={() => navigateTo('/react/dashboard')}>React 仪表盘</Button>
        </div>
      </Card>
    </div>
  )
