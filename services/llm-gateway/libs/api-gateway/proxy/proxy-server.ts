...
export class ProxyServer {
  ...
  removeHopByHopHeader(headers: IncomingHttpHeaders | OutgoingHttpHeaders) {
    ...
    for (const key in headers) {
      if (!hopByHopHeaders.has(key.toLowerCase())) {
        ...
        if (value !== undefined) newHeaders.set(key, Array.isArray(value) ? value : String(value));
      }
    }
  ...
  async forwardWebsocket(req: Request, socket: Socket, options: { headers?: NodeJS.Dict<string>; head?: Buffer } = {}) {
  ...
  async forwardRequest(req: Request, res: ServerResponse, options: { headers?: NodeJS.Dict<string> } = {}) {
    try {
      ...
      if (this.options.errorHandler) {
        this.options.errorHandler(error instanceof Error ? error : new Error(String(error)), req, res);
      } else {
        throw error;
      }
    }
