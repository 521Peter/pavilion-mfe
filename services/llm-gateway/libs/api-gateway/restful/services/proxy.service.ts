...
export class ProxyService implements OnModuleInit {
  ...
  onModuleInit(): void {
    ...
    serverInstance.getHttpServer().on("upgrade", async (request: Request, socket: Socket, head: Buffer) => {
      try {
    ...
    // 所有服务的文档首次加载结束后标记就绪，
    // 且不阻塞模块其余初始化过程。
    void Promise.allSettled(initialLoads).then(() => this.swaggerService.markInitialLoadComplete());
  }
  ...
  getServerName(url: string): string {
    ...
    if (!serverName || !this.swaggerService.apiDocs[serverName]) {
