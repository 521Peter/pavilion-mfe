...
function PlusIcon({ size = 16 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M12 5v14M5 12h14" /></svg>
}
function EditIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4Z" /></svg>
}
function TrashIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" /></svg>
}
function PlugIcon({ size = 20 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M12 22v-5" /><path d="M9 8V2" /><path d="M15 8V2" /><path d="M18 8v5a4 4 0 0 1-4 4h-4a4 4 0 0 1-4-4V8Z" /></svg>
}
function RefreshIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8" /><path d="M21 3v5h-5" /><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16" /><path d="M3 21v-5h5" /></svg>
}
function CheckCircleIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><path d="m9 11 3 3L22 4" /></svg>
}
function XCircleIcon({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" {...ic}><circle cx="12" cy="12" r="10" /><path d="m15 9-6 6M9 9l6 6" /></svg>
}

const labelClass = 'block mb-1.5 text-[13px] font-medium text-text-regular'

function Toggle({ isSelected, onChange, size = 'sm' }: { isSelected: boolean; onChange: (v: boolean) => void; size?: 'sm' | 'md' | 'lg' }) {
  return (
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="mb-4">
        ...
        <Input variant="primary" value={name} onChange={(e) => setName(e.target.value)} placeholder="如：GitHub MCP" autoFocus required fullWidth />
      </div>
      <div className="mb-4">
        <label className={labelClass}>唯一标识</label>
        <Input variant="primary" value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="如：github" required fullWidth />
      </div>
      <div className="mb-4">
        <label className={labelClass}>描述</label>
        <Input variant="primary" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="可选" fullWidth />
      </div>
      <div className="mb-4">
      ...
      {transport === 'stdio' ? (
        <>
          <div className="mb-4">
            ...
            <Input variant="primary" value={command} onChange={(e) => setCommand(e.target.value)} placeholder="如：npx" fullWidth />
          </div>
          <div className="mb-4">
            <label className={labelClass}>Args（每行一个）</label>
            <TextArea variant="primary" value={argsText} onChange={(e) => setArgsText(e.target.value)} placeholder={'-y\n@modelcontextprotocol/server-github'} rows={3} fullWidth />
          </div>
          <div className="mb-4">
            <label className={labelClass}>Env（KEY=value 格式，每行一个）</label>
            <TextArea variant="primary" value={envText} onChange={(e) => setEnvText(e.target.value)} placeholder={'GITHUB_TOKEN=ghp_xxx'} rows={3} fullWidth />
          </div>
        </>
      ) : (
        <>
          <div className="mb-4">
            ...
            <Input variant="primary" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://example.com/mcp" fullWidth />
          </div>
          <div className="mb-4">
            <label className={labelClass}>Headers（Key: Value 格式，每行一个）</label>
            <TextArea variant="primary" value={headersText} onChange={(e) => setHeadersText(e.target.value)} placeholder={'Authorization: Bearer xxx'} rows={3} fullWidth />
          </div>
        </>
      )}
      ...
      <div className="flex justify-end gap-2.5">
        <Button variant="outline" onPress={onCancel}>取消</Button>
...
export default function McpServers() {
  ...
  function openEdit(s: McpServer) { setEditing(s); setModalOpen(true) }
  ...
  return (
    <div>
      ...
      {error ? (
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(360px,1fr))] gap-[18px]">
          {servers.map((s) => (
            <Card key={s.id} variant="default" className="flex flex-col overflow-hidden transition-all hover:shadow-[0_6px_24px_rgba(0,0,0,0.06)] hover:border-primary/30">
              <div className="flex items-start gap-3.5 p-5 pb-4">
                <div className="w-[42px] h-[42px] rounded-[10px] flex items-center justify-center shrink-0" style={{ background: 'var(--primary-light)', color: 'var(--primary)' }}>
                  <PlugIcon size={22} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-[15px] font-bold text-text-primary m-0 truncate">{s.name}</h3>
                    <Chip color={transportColor(s.transport)} size="sm" variant="soft">{s.transport}</Chip>
                  </div>
              ...
              <div className="flex items-center gap-4 px-5 pb-4 text-xs text-text-muted">
                <span className="flex items-center gap-1">
                  <span className="font-bold text-text-regular">{Array.isArray(s.cachedTools) ? s.cachedTools.length : 0}</span>
                  个工具
                </span>
              ...
              <div className="mt-auto flex items-center gap-2 px-5 py-3.5 border-t border-border bg-background">
                <Button variant="outline" size="sm" onPress={() => setToolsServer(s)}>工具</Button>
                <Button variant="outline" size="sm" onPress={() => openEdit(s)}><EditIcon /> 编辑</Button>
                <Toggle isSelected={s.isActive} onChange={() => handleToggle(s)} />
                <Button variant="danger-soft" size="sm" onPress={() => handleDelete(s)} className="ml-auto"><TrashIcon /> 删除</Button>
              </div>
            </Card>
          ))}
        </div>
        ...
        <Modal.Backdrop>
          <Modal.Container>
            <Modal.Dialog className="modal-medium">
              <Modal.Header>
                <h3 className="text-base font-bold text-text-primary m-0">{editing ? '编辑 MCP Server' : '新增 MCP Server'}</h3>
              </Modal.Header>
              <Modal.Body>
                <ServerForm initial={editing ?? undefined} onSubmit={handleSubmit} onCancel={() => setModalOpen(false)} submitting={submitting} />
              </Modal.Body>
            </Modal.Dialog>
          </Modal.Container>
        </Modal.Backdrop>
      ...
      <Modal isOpen={!!toolsServer} onOpenChange={(open) => !open && setToolsServer(null)}>
        <Modal.Backdrop>
          <Modal.Container>
            <Modal.Dialog className="modal-medium">
              <Modal.Header>
                <h3 className="text-base font-bold text-text-primary m-0">工具列表{toolsServer ? ' - ' + toolsServer.name : ''}</h3>
              </Modal.Header>
              <Modal.Body>
