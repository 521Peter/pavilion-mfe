...
export class OpenApiService {
  ...
  getEndpointDetail(docUrl: string, doc: DocumentType): EndpointDetail {
    ...
    for (const router in doc.paths) {
      for (const method in doc.paths[router]) {
        ...
        const methodRouters = paths[method];
        if (!methodRouters) {
          continue;
        }
        methodRouters.push({
  ...
  async getDocument(server: string): Promise<any> {
    ...
    for (const path in document.paths) {
      for (const method in document.paths[path]) {
        if (this.apiGatewayOption.restful?.hideDocumentIds?.includes(document.paths[path][method].operationId)) {
  ...
  removeHeader(document: any) {
    for (const path in document.paths) {
      for (const method in document.paths[path]) {
        document.paths[path][method].parameters = document.paths[path][method].parameters.filter((param: any) => {
          return param.in !== "header" || !this.apiGatewayOption.excludeHeaders.includes(param.name.toLowerCase());
        });
      }
    }
  }
