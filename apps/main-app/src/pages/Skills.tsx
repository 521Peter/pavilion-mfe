...
const ChevronIcon = ({ size = 12, open }: { size?: number; open?: boolean }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
function SkillDetailPanel({ skill, onClose }: { skill: SkillDetail; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setEditing(false);
    skillApi
      .readFile(skill.name, selectedPath)
      .then(res => {
        if (!cancelled) {
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex-1 overflow-auto p-4">
          {loading ? (
            ...
            <pre className="text-[13px] font-mono text-text-primary whitespace-pre-wrap break-words m-0">
              {fileContent}
            </pre>
          )}
        </div>
        ...
        <div className="max-h-[300px] overflow-y-auto flex flex-col gap-2">
          {skills.length === 0 ? (
            <div className="py-8 text-center text-sm text-text-muted">
              该仓库没有找到 skills。
              <br />
              确保仓库结构为 <code className="text-primary">skills/&lt;name&gt;/SKILL.md</code> 或仓库根目录包含{" "}
              <code className="text-primary">SKILL.md</code>。
            </div>
          ) : (
            skills.map(skill => (
              <div
              ...
              >
                ...
                <Button
                  ...
                  isDisabled={installing === skill.name}
                >
          ...
          {skills.map(skill => (
            ...
            >
              <div className="flex items-start gap-3.5 p-5 pb-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Chip color={sourceColor(skill.source)} size="sm" variant="soft">
                      {skill.source === "remote" ? "远程" : "本地"}
                    </Chip>
                  </div>
              ...
              <div className="mt-auto flex items-center gap-2 px-5 py-3.5 border-t border-border bg-background">
                <Button variant="outline" size="sm" onPress={() => openDetail(skill.name)}>
                  浏览
                </Button>
                <Toggle isSelected={skill.isActive} onChange={() => handleToggle(skill)} />
                <Button variant="danger-soft" size="sm" onPress={() => handleDelete(skill)} className="ml-auto">
                  <TrashIcon /> 删除
                </Button>
              </div>
            </Card>
